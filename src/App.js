...
class App extends Component {
  ...
  playAgain = () => {
    this.setState(
      {viewResult: false, score: 0, timers: 60},
  ...
  viewResultDisplay = () => {
    ...
    return (
      <div className="view-result-container">
        <div className="view-result-bg">
          <img
            src="https://assets.ccbp.in/frontend/react-js/match-game-trophy.png"
            alt="trophy"
            className="trophy"
          />
          <p className="view-result-heading">YOUR SCORE</p>
          <h1 className="view-result-heading">{score}</h1>
          <div className="play-again-containers">
            <img
  ...
  render() {
    const {image, activeTabId, score, viewResult, timers} = this.state
    // console.log(image)
    const style = tabsList.map(each => {
      if (each.tabId === activeTabId) {
        return 'yes'
      }
      return 'no'
    })
    // console.log(style())
    const newList = imagesList.filter(each => each.category === activeTabId)
    return (
      <div>
          ...
          <ul className="score-timers">
            ...
            <li className="timer">
              <img
                ...
                className="timer-img"
              />
              <p className="timers">{timers} sec</p>
            </li>
          </ul>
        </div>
        <div className="bg">
          {!viewResult ? (
            <div className="bg-containers">
              <img src={image.imageUrl} alt="match" className="match" />
              <ul style={{display: 'flex'}}>
                {tabsList.map(each => (
                  <li key={each.tabId} className={`${style} nav-li`}>
                    <button
                      type="button"
                      onClick={() => this.setState({activeTabId: each.tabId})}
                    >
                      {each.displayText}
                    </button>
                  </li>
              ...
              <ul className="thumbnail-ul">
                {newList.map(each => (
                  <li className="thumbnail-li" key={each.id}>
                    <button
                      type="button"
                      onClick={() => this.checkThumbnail(each.id)}
